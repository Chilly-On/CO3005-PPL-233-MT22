...
def type_compare_expr(l, r):     # expr compare, return the side having errors
    ...
    if type(l) is VoidType:     # invalid l
        ...
        print_and_stop(["WIP IN PHRASE 3"])
...
def type_compare_stmt(l, r):     # assign stmt compare, return the side having errors
    ...
    if type(l) is VoidType:     # invalid l
        ...
        return None
    elif type(l) is FloatType:            # check float
        if type(l) is type(r) or type(r) is IntegerType:    # accept both int and float
            return None
        else:
            return 'r'
    else:                       # other types
...
class StaticChecker(Visitor):   # every testcases will have >=1 error hence must raise error by default.
    ...
    def visitId(self, ctx: Id, o):          # visit if the ID has been declared
        for scope in o:             # check each scope from in to out
            result = checkDeclare(ctx, o, VarDecl)
            if result is not None and not (type(result) is FuncDecl):          # has found declared
    ...
    def visitArrayLit(self, ctx: ArrayLit, o):
        ...
        for x in ctx.explist[1:]:
            if type(self.visit(x, o)) is not first_type:     # if one type has diff type from the first
                raise IllegalArrayLiteral(x)    # err
        return ArrayType([len(ctx.explist)], self.visit(ctx.explist[0], o))
    ...
    def visitReturnStmt(self, ctx: ReturnStmt, o):
        # check type of r
        l = type(self.Current_Function.return_type) # take cur function as l
        r = VoidType()      # if nothing, return void
        if ctx.expr:        # if have expr
            r = type(self.visit(ctx.expr, o))   # return its type
    ...
    def visitArrayType(self, ctx: ArrayType, o):  return o          # WIP IN PHRASE 3

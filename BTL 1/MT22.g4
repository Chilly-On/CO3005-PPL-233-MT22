...
stmt: vardclr | assnstmt | ifstmt | forstmt | whilestmt  //  everything for a program to run
...
// If
ifstmt: IF LC exp RC stmt (ELSE stmt)
        | IF LC exp RC stmt;

// For
forstmt: FOR LC (ID) ASSN exp COMMA exp COMMA exp RC stmt
        |FOR LC (arridx) ASSN exp COMMA exp COMMA exp RC stmt;

// While
whilestmt: WHILE LC exp RC stmt;
...
STR: ["] (~[\\\r\n'"] | '\\'[bfrnt'"\\])* ["]   {self.text = self.text[1:-1]};  /* string has to be with "", inside is
...
UNCLOSE_STRING: '"' (~[\\\r\n'"] | '\\' [bfrnt'"\\])*[\r]?[\n]?
{
	...
	elif self.text.find('\r') != -1:          # if not found newline

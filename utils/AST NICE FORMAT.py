...
tab_level = 0
def tab(n, modify = None):      # 2nd var to minus tab later
    str = ""
    for x in range(n):
        str += "\t"
...
class ArrayLit(Expr):
    ...
    def __str__(self):
        global tab_level
        tab_level += 1
        return "ArrayLit([{}{}]){}".format("\n" + tab(tab_level), (",\n" + tab(tab_level)).join([str(exp) for exp in self.explist]), tab(0, tab_level - 1))
...
class BlockStmt(Stmt):
    ...
    def __str__(self):
        ...
        old_level = tab_level
        tab_level += 1
        return "BlockStmt([{}{}]){}".format("\n" + tab(tab_level), (",\n" + tab(tab_level)).join([str(body) for body in self.body]),tab(0, old_level))
...
class IfStmt(Stmt):
    ...
    def __str__(self):
        global tab_level
        tab_level += 1
        old_tab = tab_level     # use to format Else
        return "IfStmt({}{}{}, {}){}".format(str(self.cond), ",\n" + tab(tab_level) + str(self.tstmt), tab(0, old_tab - 1), "\n" + tab(old_tab - 1, old_tab) + "Else:\n" + tab(tab_level) + str(self.fstmt) if self.fstmt else "", tab(0, old_tab - 1) if self.fstmt else "")
...
class ForStmt(Stmt):
    ...
    def __str__(self):
        global tab_level
        tab_level += 1
        return "ForStmt({}, {}, {}, {})".format(str(self.init), str(self.cond), str(self.upd), "\n" + tab(tab_level, tab_level - 1) + str(self.stmt))
...
class WhileStmt(Stmt):
    ...
    def __str__(self):
        global tab_level
        tab_level += 1
        return "WhileStmt({}, {})".format(str(self.cond), "\n" + tab(tab_level, tab_level - 1) + str(self.stmt))
...
class DoWhileStmt(Stmt):
    ...
    def __str__(self):
        global tab_level
        tab_level += 1
        return "DoWhileStmt({}, {})".format(str(self.cond), "\n" + tab(tab_level, tab_level - 1) + str(self.stmt))
